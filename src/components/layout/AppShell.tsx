...
export default function AppShell({ roleLabel, links }: Props) {
  ...
  return (
    /* The MUI theme lives here rather than around the whole app: the public
       site never renders a MUI component, and this way the library ships in
       the dashboard's own chunk instead of in a visitor's first download.

       h-screen + the scroll living on <main>: the sidebar and the mobile bars
       stay put, and a page can hand its own scrolling to a table inside it. */
    <MuiProvider>
      <div className="h-screen overflow-hidden flex" style={{ backgroundColor: C.bg }}>
        {/* Desktop sidebar */}
        ...
        <div className="flex-1 flex flex-col min-w-0 min-h-0">
          ...
          <main className={`flex-1 min-w-0 min-h-0 overflow-y-auto p-4 sm:p-6 lg:p-8 ${bottomLinks.length > 1 ? 'pb-24 lg:pb-8' : ''}`}>
            <Outlet />
          </main>
        </div>
