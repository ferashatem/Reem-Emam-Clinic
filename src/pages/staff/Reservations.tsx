...
export default function StaffReservations() {
  ...
  async function load() {
    setLoading(true)
    // Staff (reception) sees ALL reservations, not just their own
  ...
  async function onSubmit(data: ResForm) {
    ...
    try {
      ...
      if (editTarget) {
        // Keep the reservation's existing owner (admin_id) untouched on edit
        await updateReservation(editTarget.id, {
          ...data,
          price_at_booking: service?.price ?? editTarget.price_at_booking ?? 0,
        })
